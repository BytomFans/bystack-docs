...
- `String` - _public_key_, 根据指定pubkey来查询.
...
- `String` - _string_, 字符串类型的合约参数，对应的基本类型包括：`String`、`Asset`、`Hash`、`Program`、`PublicKey`.
...
- `Boolean` - _smart_contract_, 是否展示合约的UTXO，默认不显示.
...
2.  `DataArgument` 数据类型参数，`type`类型为`data`，该类型可以兼容除了`rawTxSigArgument`以外的所有合约参数类型，其值是16进制的字符串，需要注意的是`integer`整数类型是小端存储格式。

参数格式如下:(以`publickey`为例）
```
{
  "type": "data",
  "raw_data": {
