...
用小端字节序Base-128编码的无符号整型，用于可变长度选择符字符串的长度前缀。值在[0, 127]值域内被编码为1字节。值大的情况下用2或多字节。

## Integer

[LEB128](#leb128)整型，最大值为0x7fffffffffffffffff（2^63 – 1），最小为0。varint63可以装入带符号的64位整型。

## String

带有[LEB128](#leb128)前缀的二进制字符串，以字节为单位指定其长度。基础字符串的最大长度为0x7fffffff（2^31 – 1）。
空字符串编码为单字节0x00，一个字节字符串编码为两个字节0x01 0xNN，两个字节字符串编码为0x02 0xNN 0xMM，依此类推。

## String32

32字节固定长度字符串，通常用于编码[哈希](#sha3)。
...
`Pointer`编码为[String32](#string32)，并通过其ID标识另一个entry。`Pointer`限定可接受类型：`Pointer<X>`必须引用参数类型`X`。
