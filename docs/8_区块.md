...
| ID | Hash | 区块头ID，当前区块的唯一标识 |
| Transactions | []*Tx | 交易记录，区块体中的所有交易信息 |
...
| 字段 | 类型 |描述 |
| --- | --- | --- |
| Version |uint64| 区块版本号，用于跟踪软件/协议的更新 |
| Height | uint64 | 区块高度，当前区块与创世区块的距离 |
| PreviousBlockId | *Hash | 父区块哈希值，区块引用的父区块头的哈希值 |
| Timestamp | uint64 | 时间戳，该区块产生的近似时间（精确到秒的Unix时间戳） |
| TransactionsRoot | *Hash | 默克尔根，区块中包含的所有交易的哈希值生成 |
| TransactionStatusHash | *Hash | 交易状态Merkle根，区块中所有交易验证结果的哈希值生成 |
| Nonce | uint64  | 工作量证明算法的计数器 |
| Bits | uint64 | 难度，区块工作量证明算法的难度目标 |
| TransactionStatus | *TransactionStatus | 交易状态，每笔交易验证后的状态 |
